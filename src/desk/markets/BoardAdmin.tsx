...
// Officer-only controls on a binary board market: fix the wording, settle it
// YES/NO (or watch it void if a side is empty), or remove it. Board markets
// have no owner, so before the admin role existed nothing could ever pay
// them out. Mirrors the personal SettleBox but pays the PUB wallet.
export default function BoardAdmin({ code, onDeleted }: { code: string; onDeleted?: () => void }) {
  ...
  return (
    <>
      ...
      <BoardRemove code={code} onDeleted={onDeleted} />
    </>
  );
}
