...
// $1,000, so P&L is equity − 1,000, where equity is cash plus open positions
// marked at the live price — cash alone read every open bet as a loss until it
// settled. Refreshes on the shared tick.
  ...
  return (
    <div className="grid-wrap lb-wrap">
      <div className="grid-head">
        ...
        <span className="lb-note">Public board · ranked by equity, cash plus open positions at the live price · P&amp;L from $1,000</span>
      </div>
      <div className="lb">
        <div className="lb-row lb-head">
          <span>#</span><span>Member</span><span className="lb-r" title="Cash plus open positions at the live price">Equity</span>
        ...
        {(rows ?? []).map((r) => (
          <div key={r.handle} className={`lb-row ${r.isMe ? 'is-me' : ''} ${r.rank <= 3 ? 'is-top' : ''}`}>
            ...
            <span className="lb-bal mono lb-r" title={`Cash ${money(r.balance)}`}>{money(r.equity)}</span>
